// card.js — un card Dobble (entitate desenabilă)
...
export const Card = {
  ...
  asazaSimboluri: function () {
    // primul simbol în centru, restul pe un cerc
  ...
  deseneaza: function () {
    ...
    this.simboluri.forEach((sim) => {
      ...
      // fundal dacă e selectat
      if (sim.selectat) {
        noStroke();
        fill('rgba(244,208,63,0.4)');
  ...
  clicat: function (mx, my) {
    for (const sim of this.simboluri) {
      ...
      if (
        mx >= sim.x - jumatate && mx <= sim.x + jumatate &&
